...
- **engine** *[string]* - Optional. The registered collation engine to use: `collatex` (the Java microservices,
  the default), `collatex-python` (the `collatex` Python package, in-process, if it is installed), or any engine the
  services have registered with `register_engine()`. When absent, an algorithm name that matches a registered engine
  selects that engine; anything else goes to the default.
